...
class TreeToAST(Transformer):
    ...
    def compare(self, items) -> nodes.Compare:
        left = items[0]
        op = items[1]
        right = items[2]
    ...
    def not_between(self, items) -> nodes.NotBetween:
        expr = items[0]
        low = items[1]
        high = items[2]
    ...
    def select_item(self, items) -> nodes.SelectItem:
        if len(items) == 2:
            expr = items[0]
            alias = items[1]
    ...
    # select_list: "*" -> star
    def star(self, items) -> list:
        return []

    # select_list: "*" -> star | select_item ("," select_item)*
    def select_list(self, items) -> list:
        if items and isinstance(items[0], list):
            return []  # star
        return list(items)

    # table_ref: NAME ["AS"i NAME]
    def table_ref(self, items) -> nodes.TableRef:
    ...
    def join_clause(self, items) -> nodes.JoinClause:
        kind = items[0]
        table = items[1]
        on = items[2]
    ...
    def select_stmt(self, items) -> nodes.SelectStmt:
    ...
    def delete_stmt(self, items) -> nodes.DeleteStmt:
        table = items[0]
