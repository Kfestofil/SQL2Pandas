...
def _load_csv(path: Path, verbose: bool) -> dict[str, pd.DataFrame]:
    name = path.stem
    df = pd.read_csv(path)
    if verbose:
...
def _load_sql(path: Path, grammar_path: Path, verbose: bool) -> dict[str, pd.DataFrame]:
    ...
    for line in path.read_text().splitlines():
        ...
        try:
            ...
            if isinstance(ast, (CreateStmt, InsertStmt)):
                ...
                if verbose and isinstance(ast, CreateStmt):
...
def _load_pkl(path: Path, verbose: bool) -> dict[str, pd.DataFrame]:
...
def load(
    path_str: str, grammar_path: Path, verbose: bool = False
    ...
    if suffix == ".csv":
        return _load_csv(path, verbose)
    elif suffix in (".sql", ".dump"):
        return _load_sql(path, grammar_path, verbose)
    elif suffix == ".pkl":
        return _load_pkl(path, verbose)
    else:
        raise ValueError(
